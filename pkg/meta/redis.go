...
type redisOptionsResult struct {
	...
	ClientCache        bool
	ClientCacheSize    int
	ClientCacheExpiry  time.Duration
	ClientCachePreload int
}
...
func createRedisOptions(redisURL string, conf *Config) (*redisOptionsResult, error) {
	...
	// Set MaxRetries from config if provided
	if conf != nil {
	...
	return &redisOptionsResult{
		...
		ClientCachePreload: clientCachePreload,
	}, nil
}
...
func newRedisMeta(driver, addr string, conf *Config) (Meta, error) {
	...
	if result.ClientCache {
		m.cache = newRedisCache(prefix, result.ClientCacheSize, result.ClientCacheExpiry, result.ClientCachePreload)
