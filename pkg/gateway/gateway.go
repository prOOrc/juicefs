		...
		for _, fi := range fis {
			...
			entry := &minio.Entry{Name: fi.Name(),
				Info: &minio.ObjectInfo{
					Bucket:   bucket,
					Name:     fi.Name(),
					ModTime:  n.getObjModTimeByPath(n.path(bucket, prefixDir, fi.Name()), fi.ModTime()),
					Size:     fi.Size(),
					IsDir:    fi.IsDir(),
					AccTime:  fi.ModTime(),
					IsLatest: true,
...
func (n *jfsObjects) ListObjects(ctx context.Context, bucket, prefix, marker, delimiter string, maxKeys int) (loi minio.ListObjectsInfo, err error) {
	...
	getObjectInfo := func(ctx context.Context, bucket, object string, fi_ any) (obj minio.ObjectInfo, err error) {
		...
		if fi_ == nil {
			...
			if oi, ok := fi_.(*minio.ObjectInfo); ok {
				info = oi
			} else {
				fi := fi_.(*fs.FileStat)
				info = &minio.ObjectInfo{
...
func (n *jfsObjects) getObjectInfoNoFSLock(ctx context.Context, bucket, object string, info any) (oi minio.ObjectInfo, e error) {
	if info != nil {
		if oi, ok := info.(*minio.ObjectInfo); ok {
			return *oi, nil
		}
		if fi, ok := info.(*fs.FileStat); ok {
			oi = minio.ObjectInfo{
				Bucket:   bucket,
				Name:     object,
				ModTime:  fi.ModTime(),
