...
Позволяет использовать JuiceFS-мета-клиент как Go-библиотеку из внешнего модуля: экспортируемый конструктор `NewMetaClient`, возвращающий ошибку вместо завершения процесса, реализованный без изменения upstream-кода `pkg/meta`.
...
Verified-by: pkg/meta/client_test.go::TestNewMetaClient
